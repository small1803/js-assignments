...
function getRectangleString(width, height) {
    if (width < 2 || height < 2) {
        throw new Error('Minimum value of width and height is 2');
    }

    const horizontalEdge = '─'.repeat(width - 2);
    const topEdge = `┌${horizontalEdge}┐` + '\n';
...
function encodeToRot13(str) {
    for(let i = 0; i < str.length; i++) {
        ...
        str = str.substring(0, i) + String.fromCharCode(charCode) + str.substring(i + 1);
    }
...
function isString(value) {
    const preparedValue = value?.valueOf();

    return typeof preparedValue === 'string';
}
...
function getCardId(value) {
    ...
    const rank = value.substring(0, lastIndex);

    const suits = { 9827: 0, 9830: 1, 9829: 2, 9824: 3 };
    const suitsValue = suits[value.charCodeAt(lastIndex)];

    const letterRanks = { 65: 0, 74: 10, 81: 11, 75: 12};
    let rankValue = parseInt(rank) - 1;

    if (Number.isNaN(rankValue)) {
        rankValue = letterRanks[rank.charCodeAt(0)];
    }

    return rankValue + suitsValue * 13;
}
